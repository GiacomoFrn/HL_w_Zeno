...
class HilbertSpace:
    ...
    def __init__(self, n, system, flag_operators=True):
        ...
        self.sigma_y = 1j*np.array([[0, -1.], [1., 0]], dtype=complex)
    ...
    def generate_spin_operators(self):
        ...
        for i in range(self.n):
            self.X[i] = np.kron(
                np.eye(2**(i)), np.kron(self.sigma_x, np.eye(2**(self.n-i-1))))
            self.Y[i] = np.kron(
                np.eye(2**(i)), np.kron(self.sigma_y, np.eye(2**(self.n-i-1))))
            self.Z[i] = np.kron(
                np.eye(2**(i)), np.kron(self.sigma_z, np.eye(2**(self.n-i-1))))
...
class CompositeHilbertSpace:
    def __init__(self, hilbert_spaces_dict, flag_operators=True):
        # delete hilbert spaces with n 0
        hilbert_spaces_dict = {key: hs for key, hs in hilbert_spaces_dict.items() if hs.n != 0}
    ...
    def generate_composite_operators(self):
        ...
        self.X_total = self.X[0].copy()
        self.Y_total = self.Y[0].copy()
        self.Z_total = self.Z[0].copy()
